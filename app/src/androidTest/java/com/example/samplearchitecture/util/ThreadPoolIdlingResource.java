...
public class ThreadPoolIdlingResource implements IdlingResource {
    ...
    void notifyWhenIdle(final Runnable idleCallback) {
        if (idleCallback == null) throw new NullPointerException("Idle callback is null");
        IdleMonitor myMonitor = new IdleMonitor(idleCallback);
        monitor.compareAndSet(null, myMonitor);
    ...
    private class IdleMonitor {
        ...
        private IdleMonitor(final Runnable onIdle) {
            if (onIdle == null) throw new NullPointerException("onIdle is null");
            this.onIdle = onIdle;
